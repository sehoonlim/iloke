...
function ItemInquery(props) {
    ...
    const [selectedCode, setSelectedCode] = useState('pq'); // 선택된 코드
    ...
    const handleRowClick = (id, title, contents, code, comment, reg_id) => {
        ...
        navigate(`/notice/itemQueryDetail`, {
            state: { id, title, contents, comment, code, previousPage: '/notice/itemInquery', reg_id}
        });
    };
