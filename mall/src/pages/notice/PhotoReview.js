...
function PhotoReview(props) {
    ...
    const [selectedCode, setSelectedCode] = useState('pr'); // 선택된 코드
    ...
    const handleRowClick = (id, title, contents, code, comment, reg_id) => {
        ...
        navigate(`/notice/itemQueryDetail`, {
            state: { id, title, contents, comment, code, previousPage: '/notice/photoReview', reg_id}
        });
    };
  ...
  {item.reg_id === sessionStorage.getItem('userId') && sessionStorage.getItem('userType') === 'kakao'
    ...
    if (!memberId) {
        ...
        navigate(`/notice/itemWrite`, {
            state: { code: selectedCode, previousPage: '/notice/photoReview'}
        });
    }
}}>
    글쓰기
